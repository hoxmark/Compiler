...
// null, any place that uses an Object potentially could have
...
// otherwise senseless, and therefore we shouldn't return
...
println("#### here ")
assert(obj1.map(2 + _).getOrElse(3) == 3)
assert(obj2.map(_ => 3).getOrElse(3) == 3)
println("DONE")
...
// indicated by the fact that its input type is `Unit` (`()`).
