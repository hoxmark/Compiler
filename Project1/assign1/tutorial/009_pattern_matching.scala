...
random.nextString(random.nextInt(4)) match {
  case s if (s.length == 0) => println("Empty string")
...
// With te above pattern, `b` is bound to the underlying `Bar` object,
// and `b` has type `Bar`.  Now we can call `takesBar` directly.
// By using `@`, we can both destruct an object and get a handle on
...
// ...and pattern match based on a value's underlying type
...
somethingRandom match {
  ...
  case i:Int if (i==5) => println("Found 5")
  case OtherBase(Foo(_)) => println("Found an OtherBase holding a Foo")
  case OtherBase(Bar(7, true)) => println("Found an OtherBase holding a Bar, which " + "itself was holding 7")
  case b@Bar(_, true) => takesBar(b)
