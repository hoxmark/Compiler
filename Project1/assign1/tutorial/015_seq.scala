...
// something could change, but we've restricted ourselves so that this
...
// `Seq` containing another type, `map` almost assuredly is what you
...
// stating that the list is an element `a` followed by the
// rest of the list `b`.  Recall that we can nest patterns,
// so we could even pattern match on lists of length at least
...
// an empty list.  That is, if we have an empty list, then we know that
...
def lengthThree(input: List[String]): List[String] = 
  input match {
    ...
    // case of a non-empty list, where the heads length is 3
...
// for every input element, there is exactly one output element.  Instead,
...
assert(Seq(1, 2, 3).foldLeft(List[Int]())((x,y)=> y :: x) == List(3, 2, 1)) // don't use reverse
// hint: Strings have a `length` method
assert(Seq("moo", "cow", "bull").foldRight(List[Int]())((x,y) => x.length :: y) == List(3, 3, 4))
//Note to myself: The paramters change dependent on left or right.
