...
// There is also a confusing usage of the `scala` tool.
...
// With that said it's now time for some actual practice.
...
// :load 000_basics.scala

// Assuming you're in the right directory, the REPL should respond
// like so:
...
// the REPL read in `000_basics.scala`, and executed it.  It just so
// happens that `000_basics.scala` doesn't contain any executable code; it's
